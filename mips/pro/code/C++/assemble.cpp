...
void Assemble::ass_R(std::string& instruction, const std::string& operator_str, std::string& binary) {
    ...
    if (operator_str == "sll" || operator_str == "srl") {
        label_reg_imme.r2 = rt;
        label_reg_imme.r3 = rs;
        label_reg_imme.immediate = shamt;
    }
...
void Assemble::ass_I(std::string& instruction, const std::string& operator_str, std::string& binary) {
    ...
    if (operator_str == "bne" || operator_str == "beq") {
        ...
        if (reg[i] == '\0') {
            i = 0;
            while (reg[i] != '\0') {
                immediate = 10 * immediate + (reg[i] - '0');
                i++;
            }
        }
        else {
            ...
            if (it != my_label.end()) {
                ...
                char* stop;
                char* t = (char*)pc.data();
                unsigned int inniNumber = strtoul(t, &stop, 16);
                char* t1 = (char*)pp.data();
                unsigned int decimalNumber = strtoul(t1, &stop, 16);
                int num = ((long long)decimalNumber - (long long)inniNumber - 4) / 4;
                immediate = num;
    ...
    binary = deTobin(inst.opcode, 6) + deTobin(inst.rs, 5) + deTobin(inst.rt, 5) + tnum;
}
...
void Assemble::init_pc()
{
    pc = "00400000";
}


void Assemble::pc_add(int k)
{
    char* stop;
    char* t1 = (char*)pc.data();
